...
async function handleUpload() {
  ...
  transcriptionStatus.value = 'uploading'
  ...
  try {
    ...
    toast.add({
      ...
      description: `${selectedFile.value.name} has been transcribed successfully.`,
      color: 'success'
    })
...
<template>
  <UModal v-model:open="open" :ui="{ content: 'max-w-xl' }">
    <template #header>
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          <p class="text-sm text-muted mt-0.5">Upload audio or video to transcribe with AI</p>
        </div>
      </div>
    </template>
    ...
    <template #body>
      <div class="space-y-4">
        <!-- File Upload -->
        <div v-if="transcriptionStatus === 'idle'">
          <UFileUpload
        ...
        <!-- Selected File Info -->
        <div v-if="selectedFile && transcriptionStatus === 'idle'" class="flex items-center gap-3 p-3 bg-muted/10 rounded-lg">
        ...
        <!-- Options -->
        <div v-if="transcriptionStatus === 'idle' && selectedFile" class="space-y-3">
