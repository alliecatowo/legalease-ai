...
// Create a silent audio blob for waveform-only display
async function createSilentAudioBlob(duration: number): Promise<Blob> {
  // Create a minimal silent audio file (WAV format)
  // This prevents WaveSurfer from loading the actual media URL
  const sampleRate = 8000 // Low sample rate for minimal size
  ...
  view.setUint32(4, 36 + numSamples * numChannels * bytesPerSample, true) // File size - 8
  ...
  view.setUint32(40, numSamples * numChannels * bytesPerSample, true) // Data size

  // Silent audio data (all zeros)
  const silentData = new Uint8Array(numSamples * numChannels * bytesPerSample)

  // Combine header and data
  return new Blob([header, silentData], { type: 'audio/wav' })
}
...
<template>
  <div class="w-full flex flex-col gap-4">
    ...
    <div v-if="isVideo" :class="videoContainerClass" class="group">
      ...
      <div v-if="formattedDiagnostics" class="mt-2 flex flex-wrap items-center gap-3 text-[11px] text-muted border border-default rounded-lg px-3 py-2 bg-muted/10">
    ...
    <div v-if="!isVideo && formattedDiagnostics" class="flex flex-wrap items-center gap-3 text-xs text-muted border border-default rounded-lg px-3 py-2 bg-muted/10">
      <div class="flex items-center gap-1">
        <UIcon name="i-lucide-activity" class="size-3" />
        <span>RMS {{ formattedDiagnostics.rms }}</span>
      </div>
